...
class SyncWorker extends _$SyncWorker {
  ...
  Future<void> processQueue() async {
    final cloudNotifier = ref.read(cloudIntegrationProvider.notifier);
    
    // Check connectivity first
    final connectivity = ref.read(connectivityServiceProvider).value;
    if (connectivity == ConnectionState.none || connectivity == ConnectivityResult.none) {
      debugPrint('SyncWorker: Offline. Skipping sync.');
      return;
    }

    state = SyncStatus.syncing;
    cloudNotifier.setLastSyncError(null);
    ...
    if (client == null) {
      ...
      cloudNotifier.setLastSyncError('Failed to get authenticated client');
      return;
    }
    ...
    if (spreadsheetId == null) {
      debugPrint(
          'SyncWorker: Spreadsheet ID not found. Creating new spreadsheet...');
      spreadsheetId = await sheetsService.createSpreadsheet();
      if (spreadsheetId == null) {
        debugPrint(
            'SyncWorker: Failed to create/find spreadsheet. Aborting sync.');
        state = SyncStatus.failed;
        return;
      }
    ...
    final workoutItems = pendingItems
        .where((i) => i.type == 'workout' && i.workoutId != null)
        .toList();
    if (workoutItems.isNotEmpty) {
      ...
      for (final item in workoutItems) {
        final workout = await (db.select(db.workouts)
              ..where((t) => t.id.equals(item.workoutId!)))
            .getSingleOrNull();
        if (workout != null) {
          final sets = await (db.select(db.workoutSets)
                ..where((t) => t.workoutId.equals(item.workoutId!)))
              .get();
          workoutDataList.add(
              {'workout': workout, 'sets': sets, 'exerciseNames': exerciseMap});
      ...
      if (workoutDataList.isNotEmpty) {
        try {
          debugPrint(
              'SyncWorker: Appending ${workoutDataList.length} workouts...');
          await sheetsService.appendWorkoutsBatch(workoutDataList);
          await (db.update(db.syncQueue)..where((t) => t.id.isIn(successIds)))
              .write(const SyncQueueCompanion(status: Value('done')));
    ...
    final measurementItems = pendingItems
    ...
    if (measurementItems.isNotEmpty) {
      ...
      for (final item in measurementItems) {
        final m = await (db.select(db.bodyMeasurements)
              ..where((t) => t.id.equals(item.measurementId!)))
            .getSingleOrNull();
        if (m != null) {
          measurements.add(m);
          successIds.add(item.id);
      ...
      if (measurements.isNotEmpty) {
        try {
          debugPrint(
              'SyncWorker: Appending ${measurements.length} measurements...');
    ...
    await ref
        ...
        .updateSettings(currentSettings.copyWith(lastSynced: DateTime.now()));

    client.close();
    debugPrint('SyncWorker: Sync complete.');
    state = SyncStatus.success;
    cloudNotifier.updateLastSyncedAt(DateTime.now());
  }
}
