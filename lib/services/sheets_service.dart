...
class SheetsService {
  ...
  Future<void> _initializeHeaders(String id) async {
    ...
    await _api.appendValues(id, 'Exercises!A1', [
      ['Name', 'Muscle Group', 'Equipment', 'Notes']
    ]);
  ...
  Future<void> _applyFormatting(
      String spreadsheetId, List<dynamic> sheetList) async {
  ...
  Future<void> appendWorkoutsBatch(
      List<Map<String, dynamic>> workoutDataList) async {
    ...
    for (var data in workoutDataList) {
      ...
      for (var s in sets) {
        ...
        final est1rm =
            s.reps > 1 ? s.weight / (1.0278 - (0.0278 * s.reps)) : s.weight;

        allRows.add([
          dateStr,
          dayStr,
          workout.name,
          exerciseNames[s.exerciseId] ?? 'Unknown',
          s.setNumber,
          s.setType.name,
          s.weight,
          s.reps,
  ...
  Future<void> appendWorkout(Workout workout, List<WorkoutSet> sets,
      Map<int, String> exerciseNames) async {
    await appendWorkoutsBatch([
      {'workout': workout, 'sets': sets, 'exerciseNames': exerciseNames}
    ]);
  }
