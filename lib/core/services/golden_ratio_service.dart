...
class GoldenRatioService {
  ...
  static Map<String, double> calculateTargets({
    ...
    // Ideal weight based on BMI 23.5 (middle of healthy range)
    final double weight     = 23.5 * (heightCm / 100) * (heightCm / 100);

    // Ideal body fat percentage
    final double bodyFat    = isMale ? 10.0 : 20.0;

    return {
      'weight':      _round(weight),
      'bodyFat':     _round(bodyFat),
      'chest':       _round(chest),
      'waist':       _round(waist),
      'hips':        _round(hips),
