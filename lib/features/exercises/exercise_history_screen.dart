...
class _ExerciseHistoryScreenState extends ConsumerState<ExerciseHistoryScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final chartDataAsync = ref.watch(exerciseChartDataProvider(widget.exerciseId, _range));

    return Scaffold(
      backgroundColor: Theme.of(context).colorScheme.surface,
      appBar: AppBar(
        title: exerciseAsync.when(
          data: (exs) => Text(exs.firstWhere((e) => e.id == widget.exerciseId).name),
        ...
        slivers: [
              ...
              child: Column(
                ...
                children: [
                  Row(
                    ...
                    children: [
                      const Text('Progress Trend', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                      _RangeSelector(
                        current: _range,
                  ...
                  historyAsync.when(
                    data: (sessions) {
                      ...
                      if (latest.isEmpty || oldest.isEmpty) return const SizedBox.shrink();
                      
                      final latest1RM = latest.map((s) => s.weight * (1 + s.reps / 30)).reduce((a, b) => a > b ? a : b);
                      final oldest1RM = oldest.map((s) => s.weight * (1 + s.reps / 30)).reduce((a, b) => a > b ? a : b);
                      final diff = latest1RM - oldest1RM;
                      
                      return Container(
                        padding: const EdgeInsets.all(12),
                        decoration: BoxDecoration(
                          color: diff > 0 ? Colors.green.withOpacity(0.1) : Colors.grey.withOpacity(0.1),
                          borderRadius: BorderRadius.circular(12),
                          border: Border.all(color: diff > 0 ? Colors.green.withOpacity(0.3) : Colors.grey.withOpacity(0.3)),
                        ),
                        child: Row(
                          children: [
          ...
          const SliverToBoxAdapter(
            child: Padding(
              ...
              child: Text('Session History', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
            ),
          ),
          historyAsync.when(
            data: (sessions) => SliverList(
              delegate: SliverChildBuilderDelegate(
                (context, index) => _SessionCard(
                  ...
                  previousSession: index + 1 < sessions.length ? sessions[index + 1] : null,
                ),
      ...
      child: Column(
        children: [
          Row(
            ...
            children: [
              _StatItem(label: 'Max Weight', value: '${stats['maxWeight'] ?? 0}kg'),
              _StatItem(label: 'Max Reps', value: '${stats['maxReps'] ?? 0}'),
              _StatItem(label: 'Best 1RM', value: '${(stats['best1RM'] ?? 0).toStringAsFixed(1)}kg'),
            ],
          ),
          const Divider(height: 32),
          Text('Total Sets Logged: ${stats['totalSets'] ?? 0}', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold)),
        ],
      ),
    );
  }
...
class _RangeSelector extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        _RangeBtn(label: '1M', active: current.inDays == 30, onTap: () => onChanged(const Duration(days: 30))),
        _RangeBtn(label: '3M', active: current.inDays == 90, onTap: () => onChanged(const Duration(days: 90))),
        _RangeBtn(label: '6M', active: current.inDays == 180, onTap: () => onChanged(const Duration(days: 180))),
        _RangeBtn(label: 'All', active: current.inDays > 1000, onTap: () => onChanged(const Duration(days: 365 * 10))),
      ],
    );
  }
}
  ...
  Widget build(BuildContext context) {
    return InkWell(
      ...
      child: Container(
        ...
        child: Text(label, style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: active ? Colors.white : null)),
      ),
    );
  }
}
  ...
  Widget build(BuildContext context) {
    ...
    return LineChart(
      LineChartData(
        ...
        lineBarsData: [
          LineChartBarData(
            spots: data.asMap().entries.map((e) => FlSpot(e.key.toDouble(), e.value['rm'])).toList(),
            isCurved: true,
...
class _SessionCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final topSet = sets.reduce((curr, next) => curr.weight > next.weight ? curr : next);
        ...
        child: Padding(
          ...
          child: Column(
            children: [
              ...
              Row(
                children: [
                  const Icon(LucideIcons.trendingUp, size: 14, color: Colors.blue),
                  const SizedBox(width: 8),
                  Text(
                  ...
                  const SizedBox(width: 8),
                  Text('1RM: ${(topSet.weight * (1 + topSet.reps / 30)).toStringAsFixed(1)}kg', style: const TextStyle(fontSize: 11, color: Colors.grey)),
                ],
              ),
            ],
          ),
  ...
  Widget _buildTrendIndicator(WorkoutSet currentTop) {
    ...
    final prevTop = prevSets.reduce((a, b) => (a.weight * (1 + a.reps / 30)) > (b.weight * (1 + b.reps / 30)) ? a : b);
