...
class ExerciseHistoryRepository {
  ...
  Future<List<Map<String, dynamic>>> getExerciseHistory(int exerciseId) async {
    final query = _db.select(_db.workoutSets).join([
      ...
      ..where(_db.workoutSets.exerciseId.equals(exerciseId) & _db.workoutSets.completed.equals(true))
      ..orderBy([OrderingTerm(expression: _db.workouts.date, mode: OrderingMode.desc)]);
    ...
    for (final row in rows) {
      ...
      sessions.putIfAbsent(workout.id, () => {
        'date': workout.date,
  ...
  Future<List<Map<String, dynamic>>> getChartData(int exerciseId, Duration range) async {
    ...
    final query = _db.select(_db.workoutSets).join([
      ...
      ..orderBy([OrderingTerm(expression: _db.workouts.date, mode: OrderingMode.asc)]);
...
@riverpod
ExerciseHistoryRepository exerciseHistoryRepository(ExerciseHistoryRepositoryRef ref) {
