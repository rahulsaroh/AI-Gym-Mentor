...
class _PRHallOfFameScreenState extends ConsumerState<PRHallOfFameScreen> {
  ...
  Widget build(BuildContext context) {
    final prsAsync = ref.watch(recentPRsProvider); // This currently only gets last 30, but I'll use it for now
      ...
      body: Column(
        children: [
          ...
          Expanded(
            child: prsAsync.when(
              data: (prs) {
                if (prs.isEmpty) return const Center(child: Text('No PRs yet. Keep pushing!'));
              ...
              loading: () => ListView.builder(
                ...
                itemCount: 6,
                itemBuilder: (_, __) => const SkeletonCard(height: 72, margin: EdgeInsets.only(bottom: 12)),
              ),
...
class _FilterBar extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final muscles = ['All', 'Chest', 'Back', 'Shoulders', 'Biceps', 'Triceps', 'Legs', 'Core'];
  ...
  Widget build(BuildContext context) {
    return Card(
      ...
      child: ListTile(
        ...
        title: Text(pr['name'], style: const TextStyle(fontWeight: FontWeight.bold)),
        subtitle: Text('Achieved on ${DateFormat('MMM d, yyyy').format(pr['date'])}'),
        trailing: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          crossAxisAlignment: CrossAxisAlignment.end,
          children: [
            Text(pr['value'], style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: Colors.blue)),
            Text('1RM: ${(pr['rm'] as double).toStringAsFixed(1)}kg', style: const TextStyle(fontSize: 10, color: Colors.grey)),
          ],
        ),
      ),
    );
