...
class CreateEditProgramScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<CreateEditProgramScreen> createState() => _CreateEditProgramScreenState();
}
...
class _CreateEditProgramScreenState extends ConsumerState<CreateEditProgramScreen> {
  ...
  Future<void> _loadTemplate() async {
    ...
    final template = await (db.select(db.workoutTemplates)..where((t) => t.id.equals(widget.templateId!))).getSingleOrNull();
    if (template != null) {
      _nameController.text = template.name;
      _descriptionController.text = template.description ?? '';
      
      final days = await (db.select(db.templateDays)..where((t) => t.templateId.equals(widget.templateId!))..orderBy([(t) => OrderingTerm(expression: t.order)])).get();
      
      final loadedDays = <_DayData>[];
      for (var day in days) {
        final exercises = await (db.select(db.templateExercises)..where((t) => t.dayId.equals(day.id))..orderBy([(t) => OrderingTerm(expression: t.order)])).get();
        
        final exercisesData = <_ExerciseData>[];
        for (var ex in exercises) {
          exercisesData.add(_ExerciseData(
            exerciseId: ex.exerciseId,
            sets: ex.setsJson.isNotEmpty ? _parseSetsJson(ex.setsJson) : [const _SetData(3, 10, 90)],
          ));
        }
  ...
  List<_SetData> _parseSetsJson(String json) {
    try {
      final List<dynamic> list = json.isNotEmpty ? List<dynamic>.from(jsonDecode(json)) : [];
      return list.map((e) => _SetData(
        e['sets'] ?? 3, 
        e['reps'] ?? 10, 
        e['rest'] ?? 90,
        e['type'] ?? 'Straight',
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(_isEditing ? 'Edit Program' : 'Create Program', style: GoogleFonts.outfit(fontWeight: FontWeight.bold)),
        leading: IconButton(icon: const Icon(LucideIcons.x), onPressed: () => context.pop()),
        actions: [
          TextButton(
            onPressed: canSave ? _saveProgram : null,
            child: Text('Save', style: TextStyle(color: canSave ? Theme.of(context).colorScheme.primary : Colors.grey, fontWeight: FontWeight.bold)),
          ),
        ],
      ),
      body: _isLoading 
        ...
        : ListView(
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  Text('Training Days', style: GoogleFonts.outfit(fontSize: 18, fontWeight: FontWeight.bold)),
                  TextButton.icon(
                    onPressed: _addDay,
              ...
              if (_days.isEmpty)
                Center(
                  child: Column(
                    children: [
                      ...
                      const SizedBox(height: 16),
                      Text('No training days yet', style: GoogleFonts.outfit(color: Colors.grey)),
                ...
                ReorderableListView.builder(
                  ...
                  itemBuilder: (context, index) => _DayCard(
                    key: ValueKey('day_${_days[index].id ?? index}_${_days[index].name}'),
                    day: _days[index],
                    index: index,
                    ...
                    onNameChanged: (name) => setState(() => _days[index] = _days[index].copyWith(name: name)),
                    onAddExercise: () => _addExerciseToDay(index),
                    onRemoveExercise: (exIndex) {
                      final updatedExercises = List<_ExerciseData>.from(_days[index].exercises)..removeAt(exIndex);
                      setState(() => _days[index] = _days[index].copyWith(exercises: updatedExercises));
                    },
                    onUpdateExerciseSets: (exIndex, sets) {
                      final updatedExercises = List<_ExerciseData>.from(_days[index].exercises);
  ...
  Future<void> _saveProgram() async {
    ...
    try {
      if (_isEditing) {
        await (db.update(db.workoutTemplates)..where((t) => t.id.equals(widget.templateId!))).write(
        ...
        await (db.delete(db.templateDays)..where((t) => t.templateId.equals(widget.templateId!))).go();
        
        for (int i = 0; i < _days.length; i++) {
          final dayId = await db.into(db.templateDays).insert(TemplateDaysCompanion.insert(
          ...
          for (int j = 0; j < _days[i].exercises.length; j++) {
            ...
            await db.into(db.templateExercises).insert(TemplateExercisesCompanion.insert(
              dayId: dayId,
              exerciseId: ex.exerciseId,
              order: j,
              setsJson: jsonEncode(ex.sets.map((s) => {
                'sets': s.sets, 
                'reps': s.reps, 
                'rest': s.rest,
                'type': s.type,
        ...
        final templateId = await db.into(db.workoutTemplates).insert(WorkoutTemplatesCompanion.insert(
...
class _DayCardState extends ConsumerState<_DayCard> {
  ...
  void didUpdateWidget(_DayCard oldWidget) {
    ...
    if (oldWidget.day.name != widget.day.name && _nameController.text != widget.day.name) {
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      ...
      shape: RoundedRectangleBorder(
        ...
        side: widget.isExpanded ? BorderSide(color: Theme.of(context).colorScheme.primary, width: 2) : BorderSide.none,
      ),
      child: Column(
        children: [
          InkWell(
                ...
                children: [
                  if (widget.day.exercises.isEmpty)
                    Padding(
                      ...
                      child: Center(
                        child: Text('No exercises added', style: TextStyle(color: Colors.grey[600])),
                      ),
                    )
                  else
                      ...
                      return exercisesAsync.when(
                        data: (exercises) {
                          ...
                          return _ExerciseConfigRow(
                            ...
                            onUpdate: (newConfig) => widget.onUpdateExerciseSets(exIndex, [newConfig]),
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: ExpansionTile(
        ...
        title: Text(exercise.name, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 14), overflow: TextOverflow.ellipsis, maxLines: 1),
        subtitle: Text(
            ...
            children: [
              Expanded(
                child: _buildStepper(
                  label: 'Sets',
                  value: config.sets,
                  onChanged: (val) => onUpdate(_SetData(val.clamp(1, 10), config.reps, config.rest, config.type)),
                ),
              ),
              ...
              Expanded(
                child: _buildStepper(
                  label: 'Reps',
                  value: config.reps,
                  onChanged: (val) => onUpdate(_SetData(config.sets, val.clamp(1, 50), config.rest, config.type)),
                ),
              ),
            ],
          ),
            ...
            children: [
              Expanded(
                child: _buildStepper(
                  ...
                  value: config.rest,
                  step: 15,
                  onChanged: (val) => onUpdate(_SetData(config.sets, config.reps, val.clamp(0, 300), config.type)),
                ),
              ),
              const SizedBox(width: 8),
              Expanded(
                  ...
                  children: [
                    ...
                    DropdownButton<String>(
                      ...
                      items: ['Straight', 'Superset', 'Drop Set', 'AMRAP', 'Timed']
                          ...
                          .toList(),
                      onChanged: (val) => onUpdate(_SetData(config.sets, config.reps, config.rest, val ?? 'Straight')),
                    ),
                  ],
                ),
              ),
