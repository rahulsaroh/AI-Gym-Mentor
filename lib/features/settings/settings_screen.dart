...
class SettingsScreen extends ConsumerWidget {
        ...
        data: (settings) => ListView(
          ...
          children: [
            ...
            _buildSwitchTile(
              ...
              onChanged: (v) => ref.read(settingsProvider.notifier).updateSettings(settings.copyWith(timerSound: v)),
            ),
            _buildSwitchTile(
              context,
              title: 'Timer Vibration',
              value: settings.timerVibration,
              onChanged: (v) => ref.read(settingsProvider.notifier).updateSettings(settings.copyWith(timerVibration: v)),
            ),
            _buildSwitchTile(
              context,
              title: 'Background Notifications',
              value: settings.backgroundNotification,
              onChanged: (v) => ref.read(settingsProvider.notifier).updateSettings(settings.copyWith(backgroundNotification: v)),
            ),
            ...
            _buildSwitchTile(
              ...
              onChanged: (v) => ref.read(settingsProvider.notifier).updateSettings(settings.copyWith(showRpe: v)),
            ),
            _buildSwitchTile(
              context,
              ...
              onChanged: (v) => ref.read(settingsProvider.notifier).updateSettings(settings.copyWith(showPreviousData: v)),
            ),
            ...
            _buildTile(
              ...
              onTap: () => ref.read(backupServiceProvider.notifier).exportToLocalFile(),
            ),
            _buildTile(
              context,
              ...
              onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const CsvExportScreen())),
            ),
            _buildTile(
              context,
              ...
              onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const ImportWizardScreen())),
            ),
  ...
  Widget _buildTile(BuildContext context, {required String title, String? subtitle, required IconData icon, VoidCallback? onTap}) {
    return ListTile(
      leading: Icon(icon, color: Theme.of(context).colorScheme.onSurfaceVariant),
      title: Text(title),
      subtitle: subtitle != null ? Text(subtitle, style: TextStyle(color: Theme.of(context).colorScheme.primary)) : null,
      trailing: const Icon(Icons.chevron_right, size: 20),
      onTap: onTap,
    );
  }

  Widget _buildSwitchTile(BuildContext context, {required String title, String? subtitle, required bool value, required ValueChanged<bool> onChanged}) {
  ...
  Widget _buildThemeSegmented(BuildContext context, WidgetRef ref, SettingsState settings) {
    return Padding(
  ...
  Widget _buildFontSizeSegmented(BuildContext context, WidgetRef ref, SettingsState settings) {
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
  ...
  Widget _buildAccentColorPicker(BuildContext context, WidgetRef ref, SettingsState settings) {
  ...
  Widget _buildRestTimerSection(BuildContext context, WidgetRef ref, SettingsState settings) {
    return Column(
      children: [
        _buildStepperTile(
          context, 
          ...
          (v) => ref.read(settingsProvider.notifier).updateRestTime('straight', v)
        ),
        _buildStepperTile(
          context, 
          'Supersets', 
          settings.restTimeSuperset, 
          (v) => ref.read(settingsProvider.notifier).updateRestTime('superset', v)
        ),
        _buildStepperTile(
          context, 
          'Dropsets', 
          settings.restTimeDropset, 
          (v) => ref.read(settingsProvider.notifier).updateRestTime('dropset', v)
        ),
      ],
    );
  }

  Widget _buildStepperTile(BuildContext context, String title, int seconds, ValueChanged<int> onChanged) {
    return ListTile(
      title: Text(title),
      trailing: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          IconButton(onPressed: seconds > 30 ? () => onChanged(seconds - 30) : null, icon: const Icon(Icons.remove_circle_outline, size: 20)),
          Container(
            width: 60,
            alignment: Alignment.center,
            child: Text('${seconds}s', style: const TextStyle(fontWeight: FontWeight.bold)),
          ),
          IconButton(onPressed: seconds < 300 ? () => onChanged(seconds + 30) : null, icon: const Icon(Icons.add_circle_outline, size: 20)),
        ],
      ),
    );
  }

  Widget _buildBarbellWeightInput(BuildContext context, WidgetRef ref, SettingsState settings) {
    return ListTile(
      title: const Text('Barbell Weight'),
      trailing: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          Text('${settings.barbellWeight}kg', style: TextStyle(color: Theme.of(context).colorScheme.primary, fontWeight: FontWeight.bold)),
  ...
  Widget _buildAutoIncrementDropdown(BuildContext context, WidgetRef ref, SettingsState settings) {
    ...
    return ListTile(
      ...
      trailing: DropdownButton<double>(
        ...
        onChanged: (v) => ref.read(settingsProvider.notifier).updateSettings(settings.copyWith(autoIncrement: v ?? 0.0)),
      ),
    );
  }
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          FilledButton(
            onPressed: () {
              ref.read(settingsProvider.notifier).updateName(controller.text.trim());
  ...
  void _showExperienceDialog(BuildContext context, WidgetRef ref, SettingsState settings) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        ...
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: ExperienceLevel.values.map((e) => RadioListTile<ExperienceLevel>(
            title: Text(_capitalize(e.name)),
            value: e,
            groupValue: settings.experienceLevel,
            onChanged: (val) {
              ref.read(settingsProvider.notifier).updateSettings(settings.copyWith(experienceLevel: val));
              Navigator.pop(context);
            },
          )).toList(),
        ),
  ...
  void _showWeightInputDialog(BuildContext context, WidgetRef ref, SettingsState settings) {
    final controller = TextEditingController(text: settings.barbellWeight.toString());
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Barbell Weight'),
        ...
        actions: [
          ...
          FilledButton(
            onPressed: () {
              ...
              ref.read(settingsProvider.notifier).updateSettings(settings.copyWith(barbellWeight: val));
              Navigator.pop(context);
            },
            child: const Text('Save'),
          ),
  ...
  void _showDeleteConfirmation(BuildContext context, WidgetRef ref, String type) {
        ...
        builder: (context, setState) => AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              const Text('Type DELETE to confirm:', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
              const SizedBox(height: 8),
              TextField(
                controller: controller,
          ...
          actions: [
            TextButton(onPressed: () => Navigator.pop(context), child: const Text('Cancel')),
            FilledButton(
              onPressed: controller.text == 'DELETE' ? () async {
  ...
  Widget _buildDangerTile(BuildContext context, {required String title, required VoidCallback onTap}) {
    return ListTile(
      title: Text(title, style: const TextStyle(color: Colors.red, fontWeight: FontWeight.bold)),
      trailing: const Icon(Icons.warning_amber_rounded, color: Colors.red, size: 20),
      onTap: onTap,
    );
  }
