...
class _ImportWizardScreenState extends ConsumerState<ImportWizardScreen> {
      ...
      body: Stepper(
        ...
        steps: [
          ...
          Step(
              ...
              isActive: _currentStep == ImportStep.progress,
  ...
  int _getStepIndex() {
    switch (_currentStep) {
      case ImportStep.select:
        return 0;
      case ImportStep.validate:
        return 1;
      case ImportStep.resolve:
        return 2;
  ...
  Widget _buildFileSelect() {
    return Column(
      children: [
        const Icon(LucideIcons.fileDigit, size: 64, color: Colors.blue),
        const SizedBox(height: 24),
        const Text('Select a .json backup file generated by GYM Kilo.',
      ...
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context)
              .showSnackBar(SnackBar(content: Text('Invalid file: $e')));
        }
      }
    }
  }
      ...
      children: [
        ...
        Row(
          ...
          children: [
            TextButton(
                onPressed: () =>
                    setState(() => _currentStep = ImportStep.select),
                child: const Text('Back')),
            ElevatedButton(
                onPressed: _checkConflicts, child: const Text('Next')),
          ],
        ),
      ],
    );
  ...
  Future<void> _checkConflicts() async {
    ...
    final existingWorkoutDates =
        workouts.map((w) => w.date.toIso8601String()).toSet();

    int duplicates = 0;
    final importWorkouts = _importData!['workouts'] as List;
    for (var w in importWorkouts) {
      if (existingWorkoutDates.contains(w['date'])) {
        duplicates++;
      }
    }
  ...
  Widget _buildConflictResolution() {
    return Column(
      ...
      children: [
        Text('Detected $_duplicateCount duplicate workout(s).',
            style: const TextStyle(
        ...
        RadioListTile(
            ...
            subtitle: const Text('Only import new data (Recommended)'),
            value: 'skip',
        ...
        RadioListTile(
            ...
            subtitle:
                const Text('Overwrite existing workouts with backup data'),
            value: 'replace',
            groupValue: _conflictResolution,
            onChanged: (v) => setState(() => _conflictResolution = v!)),
        RadioListTile(
            ...
            subtitle:
                const Text('Keep everything, even if duplicates are created'),
            value: 'all',
            groupValue: _conflictResolution,
            onChanged: (v) => setState(() => _conflictResolution = v!)),
        const SizedBox(height: 32),
        Row(
          ...
          children: [
            TextButton(
                onPressed: () =>
                    setState(() => _currentStep = ImportStep.validate),
                child: const Text('Back')),
            ElevatedButton(
                onPressed: _startImport, child: const Text('Start Import')),
          ],
        ),
      ],
    );
  ...
  Future<void> _startImport() async {
    ...
    await db.transaction(() async {
      ...
      for (var w in importWorkouts) {
        ...
        if (isDuplicate && _conflictResolution == 'skip') {
          continue;
        }

        if (isDuplicate && _conflictResolution == 'replace') {
          await (db.delete(db.workouts)
                ..where((t) => t.date.equals(workoutDate)))
              .go();
        }
        ...
        final setsForWorkout =
            importSets.where((s) => s['workout_id'] == w['id']).toList();
  ...
  Future<bool> _isWorkoutDuplicate(
      AppDatabase db, Map<String, dynamic> w) async {
    final date = DateTime.parse(w['date']);
    final match = await (db.select(db.workouts)
